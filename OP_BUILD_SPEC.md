...
It does not compile source code by itself. It reads `holon.yaml`,
selects the declared runner, executes the minimum required sequence,
and reports the primary artifact path.
...
The runner namespace should distinguish between leaf runners and
...
- `go-module`
- `cmake`
- future: `dart-package`, `cargo`, `gradle`, `swift-package`, `dotnet`
...
- `type: holon` means the path must contain its own `holon.yaml`
...
This spec reserves `dart-package` for Flutter or Dart application
builds, but does not require it for the first implementation.

Composite recipes may still invoke Flutter through `exec` until the
runner exists.
